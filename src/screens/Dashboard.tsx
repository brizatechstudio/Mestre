  ...
  return <div className="stack-lg">
    ...
    <section className="panel">
      ...
      {quotes.length ? <div className="table-wrap"><table><thead><tr><th>Número</th><th>Cliente</th><th>Status</th><th>Total</th><th>Atualizado</th><th></th></tr></thead><tbody>
        {quotes.slice().sort((a,b)=>b.updatedAt.localeCompare(a.updatedAt)).slice(0,6).map(q => <tr key={q.id}><td><b>{q.number}</b></td><td>{q.clientSnapshot?.name || clients.find(c=>c.id===q.clientId)?.name || 'Sem cliente'}</td><td><Status status={q.status}/></td><td>{money(quoteTotal(q.services,q.materials,q.discount,q.taxPercent??0,q.profitMarginPercent??0).total)}</td><td>{shortDate(q.updatedAt)}</td><td><button className="icon-button" onClick={() => onOpen(q.id)}><Icon name="arrow"/></button></td></tr>)}
