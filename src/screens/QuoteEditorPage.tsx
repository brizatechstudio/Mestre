...
function QuoteItemsSection({number,title,kind,items,catalog,addCatalog,addBlank,updateItem,removeItem,hourlyCostBase,onAddLaborBase}:{number:string;title:string;kind:'services'|'materials';items:QuoteItem[];catalog:CatalogItem[];addCatalog:(k:'services'|'materials',id:string)=>void;addBlank:(k:'services'|'materials')=>void;updateItem:(k:'services'|'materials',id:string,p:Partial<QuoteItem>)=>void;removeItem:(k:'services'|'materials',id:string)=>void;hourlyCostBase?:number;onAddLaborBase?:()=>void}){
  ...
  return <section className="panel quote-section"><div className="quote-section__row"><SectionTitle number={number} title={title}/><div className="add-item-controls"><select value={choice} onChange={e=>{setChoice(e.target.value);if(e.target.value){addCatalog(kind,e.target.value);setChoice('')}}}><option value="">Adicionar do cadastro...</option>{catalog.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select><button className="button button--ghost button--small" onClick={()=>addBlank(kind)}><Icon name="plus" size={15}/> Manual</button></div></div>
    ...
    {items.length?<div className="quote-items"><div className="quote-items__head"><span>Descrição</span><span>Qtd.</span><span>Un.</span><span>Preço unit.</span><span>Total</span><span/></div>{items.map(item=><div className="quote-item" key={item.id}><div className="quote-item__field quote-item__description" data-label="Descrição"><div className="input-with-action"><input value={item.name} onChange={e=>updateItem(kind,item.id,{name:e.target.value})} placeholder={kind==='services'?'Descrição do serviço':'Descrição do material'}/><MicButton onText={t=>updateItem(kind,item.id,{name:t})}/></div></div><label className="quote-item__field" data-label="Quantidade"><input type="number" min="0" step="0.01" value={item.quantity} onChange={e=>updateItem(kind,item.id,{quantity:Number(e.target.value)})}/></label><label className="quote-item__field" data-label="Unidade"><select value={item.unit} onChange={e=>updateItem(kind,item.id,{unit:e.target.value as Unit})}>{units.map(u=><option key={u}>{u}</option>)}</select></label><label className="quote-item__field" data-label="Preço unitário"><input type="number" min="0" step="0.01" value={item.unitPrice} onChange={e=>updateItem(kind,item.id,{unitPrice:Number(e.target.value)})}/></label><div className="quote-item__field quote-item__total" data-label="Total"><strong>{money(item.quantity*item.unitPrice)}</strong></div><button className="icon-button danger quote-item__remove" aria-label={`Remover ${item.name || 'item'}`} onClick={()=>removeItem(kind,item.id)}><Icon name="trash"/></button></div>)}</div>:<div className="inline-empty"><p>Nenhum item adicionado.</p><button className="link-button" onClick={()=>addBlank(kind)}>+ Adicionar manualmente</button></div>}
  </section>
}
